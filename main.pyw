...
class App(ctk.CTk):
    def __init__(self):
        super().__init__(); self.title(f"RAE Turbo - v{VERSAO_ATUAL}"); self.geometry("620x610"); self.minsize(620,610)
    ...
    def _montar_ui(self):
        ...
        ctk.CTkLabel(self,text="RAE Turbo",font=ctk.CTkFont(size=28,weight="bold")).grid(row=0,column=0,pady=(25,4))
    ...
    def abrir_sobre(self):
        w=ctk.CTkToplevel(self);w.title("Sobre o RAE Turbo");w.geometry("500x380");w.resizable(False,False);w.grab_set()
        ctk.CTkLabel(w,text="RAE Turbo",font=ctk.CTkFont(size=26,weight="bold")).pack(pady=(30,5));ctk.CTkLabel(w,text=f"Versão {VERSAO_ATUAL}").pack(pady=(0,20))
